...
int main(void)
{
    ...
    // Timer1 PWM OC1A (PD5), Non-inverting
    Timer1_init(TIMER_MODE_FAST_PWM, TIMER_PRESCALER_64, PWM_NON_INVERTING, TIMER1_OC1A);
    Timer1_setFrequency(1000, TIMER1_OC1A);
    Timer1_setDuty(75, TIMER1_OC1A);

    // Timer1 PWM OC1B (PD4), Inverting
    Timer1_init(TIMER_MODE_FAST_PWM, TIMER_PRESCALER_64, PWM_INVERTING, TIMER1_OC1B);
    Timer1_setFrequency(1000, TIMER1_OC1B);
    Timer1_setDuty(60, TIMER1_OC1B);

    // Timer2 PWM Non-inverting
    Timer2_init(TIMER_MODE_FAST_PWM, TIMER_PRESCALER_64, PWM_NON_INVERTING);
    Timer2_setFrequency(2000);
    Timer2_setDuty(80);

    while(1)
    {
        // PWM automatically runs on OC pins
    }
}
*/